...
MAIN_STAT = STR  # Change this to your class's main stat (STR/DEX/INT/LUK)
SECONDARY_STAT = DEX  # Change this to your class's secondary stat (STR/DEX/INT/LUK)
...
WEAPON = "ATTACK"  # Set to "ATTACK" or "MAGIC"
TARGET_ATTACK = 157  # The target weapon/magic attack value
...
def calculate_flame_score(stats):
    ...
    weapon_att_value = stats["weapon_attack"] * 3
    all_stat_value = stats["all_stat_percent"] * 10
    secondary_stat_value = stats["secondary_stat"] / 10
...
def weapon_flaming():
    try:
        ...
        attack_type = "Weapon Attack" if WEAPON == "ATTACK" else "Magic Attack"
        stat_key = 'weapon_attack' if WEAPON == "ATTACK" else 'magic_attack'
        
        print(f"\nLooking for {attack_type}: +{TARGET_ATTACK}")
        
        while True:  # Keep running until we get target value
            # Check for stop key
            if check_stop_key():
                print("\nShift+Esc detected. Stopping script...")
                break
                
            # Capture after box (we only need to check the new stats)
            after_image = capture_stat_box(window_rect, is_before=False)
            after_stats = extract_stats(after_image, is_before=False)
            
            # Check if we got the target attack value
            current_attack = after_stats[stat_key]
            print(f"\nCurrent {attack_type}: +{current_attack}")
            
            if current_attack == TARGET_ATTACK:
                print(f"\nFound target {attack_type}: +{TARGET_ATTACK}!")
                print("Stopping script...")
                break
            else:
                print(f"Rerolling... (looking for +{TARGET_ATTACK})")
                reroll(window_rect)
...
def main():
    try:
        print("\nPress Shift+Esc at any time to stop the script\n")
        
        if WEAPON == "ATTACK":
            print("Trigger weapon function")
            weapon_flaming()
        else:
            print("Trigger armour function")
            armour_flaming()
