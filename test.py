...
def capture_stat_box(rect, is_before=True):
    ...
    screenshot = ImageGrab.grab(bbox=(box_x, box_y, box_x + box_width, box_y + box_height))
...
def calculate_flame_score(stats):
    ...
    flame_score = main_stat_value + weapon_att_value + all_stat_value + secondary_stat_value
...
def print_flame_scores(before_stats, after_stats, before_score, after_score):
    ...
    print(f"Total Score: {before_score:<{width-13}}|  Total Score: {score_color}{after_score}{RESET}")
    print("=" * (width * 2 + 3))
